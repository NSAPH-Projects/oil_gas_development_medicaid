...
mod_event_adj_boom = feols(adjustment_reaction_hospitalizations ~ i(time_to_treat, treat) | 
                                residence_county + year, 
                              data = merge_prepost)

mod_event_anx_boom = feols(anxiety_disorders_hospitalizations ~ i(time_to_treat, treat) | 
                                residence_county + year, 
                              data = merge_prepost)

mod_event_att_boom = feols(attention_disorders_hospitalizations ~ i(time_to_treat, treat) | 
                                residence_county + year, 
                              data = merge_prepost)

mod_event_mood_boom = feols(mood_disorders_hospitalizations ~ i(time_to_treat, treat) | 
                                 residence_county + year, 
                               data = merge_prepost)

mod_event_pers_boom = feols(personality_disorders_hospitalizations ~ i(time_to_treat, treat) | 
                                 residence_county + year, 
                               data = merge_prepost)

mod_event_schiz_boom = feols(schizophrenia_psychotic_disorders_hospitalizations ~ i(time_to_treat, treat) | 
                                  residence_county + year, 
                                data = merge_prepost)

mod_event_alc_boom = feols(alcohol_disorders_hospitalizations ~ i(time_to_treat, treat) | 
                                residence_county + year, 
                              data = merge_prepost)

mod_event_sub_boom = feols(substance_disorders_hospitalizations ~ i(time_to_treat, treat) | 
                                residence_county + year, 
                              data = merge_prepost)

mod_event_suic_boom = feols(suicide_self_harm_hospitalizations ~ i(time_to_treat, treat) | 
                                 residence_county + year, 
                               data = merge_prepost)
...
mod_event_mh_bust = fenegbin(mental_health_hospitalizations ~ i(time_to_treat2, treat) | 
                   residence_county + year, 
                   data = merge_prepost)

mod_event_adj_bust = fenegbin(adjustment_reaction_hospitalizations ~ i(time_to_treat2, treat) | 
                       residence_county + year, 
                     data = merge_prepost)

mod_event_anx_bust = fenegbin(anxiety_disorders_hospitalizations ~ i(time_to_treat2, treat) | 
                       residence_county + year, 
                     data = merge_prepost)

mod_event_att_bust = fenegbin(attention_disorders_hospitalizations ~ i(time_to_treat2, treat) | 
                              residence_county + year, 
                            data = merge_prepost)

mod_event_mood_bust = fenegbin(mood_disorders_hospitalizations ~ i(time_to_treat2, treat) | 
                              residence_county + year, 
                            data = merge_prepost)

mod_event_pers_bust = fenegbin(personality_disorders_hospitalizations ~ i(time_to_treat2, treat) | 
                              residence_county + year, 
                            data = merge_prepost)

mod_event_schiz_bust = fenegbin(schizophrenia_psychotic_disorders_hospitalizations ~ i(time_to_treat2, treat) | 
                              residence_county + year, 
                            data = merge_prepost)

mod_event_alc_bust = fenegbin(alcohol_disorders_hospitalizations ~ i(time_to_treat2, treat) | 
                               residence_county + year, 
                             data = merge_prepost)

mod_event_sub_bust = fenegbin(substance_disorders_hospitalizations ~ i(time_to_treat2, treat) | 
                             residence_county + year, 
                           data = merge_prepost)

mod_event_suic_bust = fenegbin(suicide_self_harm_hospitalizations ~ i(time_to_treat, treat) | 
                             residence_county + year, 
                           data = merge_prepost)
...
# Create variables that flag the first boom year
test2 <- test %>% 
  dplyr::group_by(residence_county) %>%
  dplyr::mutate(
...
# Isolate first year and county
test3 <- unique(test2[,c("residence_county","first")])


# Merge first year into merge dataset (and reorder so it's easy to check)
test_sub <- test3[which(test3$residence_county %in% merge$residence_county),]
merge_prepost <- merge(merge, test_sub, by = c("residence_county"), all = TRUE)
merge_prepost <- merge_prepost[order(merge_prepost$residence_county, merge_prepost$year),]

#  Create indicator of which states received treatment
merge_prepost$treat <- ifelse(is.na(merge_prepost$first), 0, 1)

## Create indicator of time to treatment
merge_prepost$time_to_treat <- ifelse(merge_prepost$treat==1, merge_prepost$year - merge_prepost$first, 0)

# Make the -1 period the central period
merge_prepost$time_to_treat2 <- merge_prepost$time_to_treat + 1
merge_prepost$time_to_treat2[which(is.na(merge_prepost$first))] <- 0

merge_prepost$time_to_treat2 <- relevel(as.factor(merge_prepost$time_to_treat2),ref = "0")



mod_event_mh_boom = fenegbin(mental_health_hospitalizations ~ i(time_to_treat2, treat) | 
                            residence_county + year, 
                          data = merge_prepost)
...
#table 4
#Zero inflated models for rare outcomes